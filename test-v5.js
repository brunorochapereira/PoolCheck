...
assert.equal(C.ENGINE_VERSION,'6.0.0');
...
// RC2: CYA crítico deve preceder recomendações informativas.
r=C.analyse({ph:7.4,freeChlorine:2,alkalinity:200,cya:200,totalChlorine:null,hardness:null,waterTemp:25},pool,products,{});
assert.equal(r.allIssues[0].key,'cya-critical','CYA crítico deve aparecer antes de alcalinidade elevada');
assert.ok(r.allIssues.findIndex(x=>x.key==='cya-critical')<r.allIssues.findIndex(x=>x.key==='alk-high'));
const linkedPlan=C.planFrom(r,'analysis-rc2');assert.equal(linkedPlan.analysisId,'analysis-rc2');
